...
def torontomu_faculty():
    ...
    university = "Toronto Metropolitan University, CS"
    country = "Canada"
    profs = []
    ...
    for section in professor_sections:
        try:
            ...
            profile_relative_url = section.find('a', class_='embed-responsive-item')['href']
            ...
            name_tag = profile_soup.find("h1", class_="name")
            name_parts = name_tag.find_all("span") if name_tag else []
            name = " ".join([part.get_text(strip=True) for part in name_parts]) if name_parts else "Not Found"
            ...
            research_tags = profile_soup.select('.res-text p strong')
    ...
    print("Toronto Metropolitan University data extraction complete")
    return profs
