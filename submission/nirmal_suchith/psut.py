...
def psut_faculty():
    ...
    faculty_directory_url = f"{base_url}/en/staff/professor/computer-science/department/all-staff"
    driver.get(faculty_directory_url)
    driver.implicitly_wait(10)

    # Parse the main faculty directory page to extract sections for professors
    soup = BeautifulSoup(driver.page_source, "html.parser")
    faculty_sections = soup.select('.row.member-board .col-lg-3.col-md-6')

    # File setup for output
    txt_filename = "psut_faculty.txt"
    csv_filename = "psut_faculty.csv"
    ...
    university = "Princess Sumaya University for Technology, CS"
    country = "Jordan"
    ...
    # Process each faculty member's section
    for section in faculty_sections:
        try:
            ...
            email_tag = profile_soup.find("p", string=re.compile("Email:"))
            email = None
            if email_tag:
                # Find the next sibling <p> tag containing the email
                email_tag = email_tag.find_next("p")
                email = email_tag.get_text(strip=True) if email_tag else "Not Found"

            # Extract professor's research areas (bio paragraph)
            bio_tag = profile_soup.find("p", string=re.compile("received a Ph.D."))
            research_area_text = bio_tag.get_text(strip=True) if bio_tag else "Not Found"

            # Check if research matches any keywords
            if any(re.search(keyword, research_area_text, re.IGNORECASE) for keyword in keyword_list):
                # Save the relevant professor's details
                txt_file.write(f"Name: {name}\nEmail: {email}\nWebsite: {profile_url}\nUniversity: {university}\nCountry: {country}\nResearch Areas: {research_area_text}\n\n")
                csvwriter.writerow([university, country, name, email, profile_url, research_area_text])
