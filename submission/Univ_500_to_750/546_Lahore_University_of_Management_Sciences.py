...
def lums():
    url = "https://lums.edu.pk/faculty-resources"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Lahore University of Management Sciences (LUMS).csv"
        ...
        univ_name = "Lahore University of Management Sciences (LUMS)"
        country = "Pakistan"

        garbage_emails = [] # add garbage emails to this list

        variables = [csvwriter1, csvwriter2, univ_name, country, garbage_emails]
        d = soup.find('div', {'class': 'item-list'})
        dd = d.find('ul', {'class': 'faculty-views-user'})
        profs = dd.find_all('li')
        
        #<div class="profile-faculty-users-full-name"><span class="name-title">Dr. </span><span class="real-name">Ali Cheema</span></div>
        #<div class="profile-faculty-users-link"><a href="/lums_employee/441">Profile</a></div>
        for i in profs:
            name_div = i.find('div', {'class': 'profile-faculty-users-full-name'})
            name = name_div.find('span', {'class': 'real-name'}).get_text()
            link = "https://lums.edu.pk" + i.find('div', {'class': 'profile-faculty-users-link'}).find('a').get('href')
            email = "Not Found"
            print(name, link)
            try:
