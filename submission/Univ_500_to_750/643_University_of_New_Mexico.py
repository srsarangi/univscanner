...
def unm():
    url = "https://www.cs.unm.edu/directory/index.html"
    response = requests.get(url)
    if response.status_code == 200:

        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "University of New Mexico.csv"
        ...
        univ_name = "University of New Mexico"
        country = "United States"

        garbage_emails = ['csinfo@cs.unm.edu','cssupport@cs.unm.edu'] # add garbage emails to this list

        variables = [csvwriter1, csvwriter2, univ_name, country, garbage_emails]

        d = soup.find_all('div', {'class': 'single-person-entry computer-science'})
        for prof in d:
            h4 = prof.find('h4')
            if h4 == None:
                continue
            ...
            link = "https://www.cs.unm.edu/directory" +a.get('href')
            print(name,link)
            email = "Not Found"
            try:
