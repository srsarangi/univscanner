...
def drexel():
    url = "https://drexel.edu/cci/academics/computer-science-department/faculty/"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable

        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Drexel University.csv"
        ...
        # cci-table
        d = soup.find('div', {'class': 'cci-table'})
        if d == None:
            print("None")
            return
        dd = d.find_all('div', {'class': 'row'})
        for i in dd:
            name_div = i.find('div', {'id': 'name'})
            if name_div == None:
                continue
            a = name_div.find('a')
            if a == None:
            ...
            link = "https://drexel.edu/cci"+ a.get('href')
            # print(name, link)
            email = "Not Found"
            # a1 = i.find('a', text="Website)
            email_a = i.find('a',{'class':'email-link'})
            if email_a != None:
                # email = email_a.get_text()
