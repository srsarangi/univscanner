...
def brandeis():
    url = "https://www.brandeis.edu/computer-science/people/index.html"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Brandeis University.csv"
        ...
        univ_name = "Brandeis University"
        country = "United States"

        garbage_emails = [] # add garbage emails to this list

        variables = [csvwriter1, csvwriter2, univ_name, country, garbage_emails]
        profs = soup.find_all('div',{'class':'faculty__listing'})
        print(len(profs))
        for prof in profs:
            a = prof.find('a',{'class':'faculty__listing__name'})
            if a == None:
                continue
            link = a.get('href')
