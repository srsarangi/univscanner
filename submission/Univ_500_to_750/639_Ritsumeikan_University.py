...
def ritsumeikan():
    url = "https://www.ritsumei.ac.jp/se/re/ec/index-e.html"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        print('scrappable')
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Ritsumeikan University.csv"
        ...
        univ_name = "Ritsumeikan University"
        country = "Japan"

        garbage_emails = [] # add garbage emails to this list

        variables = [csvwriter1, csvwriter2, univ_name, country, garbage_emails]
        d1 = soup.find('table', {'class': 'solid center margin_bottom'})
        d2 = soup.find('table', {'class': 'solid margin_bottom'})
        d3 = soup.find_all('ul', {'class': 'english'})
        d3.append(d1)
        d3.append(d2)
        print(len(d3))
        for i in d3:
