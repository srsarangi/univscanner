...
def georgia():
    url = "https://www.fcs.uga.edu/people/dept/34"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "The University of Georgia.csv"
        ...
        univ_name = "The University of Georgia"
        country = "United States"

        garbage_emails = [] # add garbage emails to this list

        variables = [csvwriter1, csvwriter2, univ_name, country, garbage_emails]
        d = soup.find('ul', {'class': 'list-table d-table table', 'id': 'directory'})
        # print(len(d))

        
        # get the above html element from soup
        dd = d.find_all('li', {'class': 'd-table-row vcard'})
        # find name, email, link
        for i in dd:
            div = i.find_all('div', {'class': 'd-md-table-cell'})
            if len(div) < 3:
                continue
            # return 
            name_div = div[1]
            ...
            # p = name_div.find('p', {'class': 'fn n h5'})

            a = name_div.find('a')
            # print(a)
            name = a.get_text()
            print(name)
            link = "https://www.fcs.uga.edu"+ a.get('href')
            email = "Not Found"
            # check if link is valid or not
            email_div = div[2]
            email = email_div.find('a', {'class':'email no-icon'}).get_text()
