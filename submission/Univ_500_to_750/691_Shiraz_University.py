...
# JUST TEMPLATE
def shiraz():
    url = "https://cse.shirazu.ac.ir/faculty"
    return
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Shiraz University.csv"
        ...
        d = soup.find('div',{'class':'journal-content-article'})
        if d == None:
            print("d is None")
            return
...
def get_email(variables,garbage_emails,name,link,email,page_resp):
    ...
    key_words = ['operating systems','Embedded System','Embedded','embedded system', 'Operating Systems','operating system','Operating System','embedded systems',"Embedded Systems",'kernel','Microcontroller Systems','Integrated Systems','Embedded Computing','Embedded Software','unix','linux']
