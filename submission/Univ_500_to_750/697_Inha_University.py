...
def inha():
    url = "https://inha.uz/about/professors/socie-school-of-computer-science-and-information-engineering/"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Inha University.csv"
        ...
        d = soup.find_all('div',{'class':'col prof medium-4 small-12 large-4'})
        ...
        for prof in d:
            ...
            if len(a) ==3:
                link = url
                name = a[1].get_text().strip()
                email = a[2].get('href')[7:]
                p = prof.find_all('p',{'class':'peducation'})
                final_text = ""
                for i in p:
                    p_text = i.get_text().strip()
                    final_text += p_text
                p_s = prof.find_all('p',{'class':'psubject'})
                for i in p_s:
                    p_text = i.get_text().strip()
                    final_text += p_text
                ...
                for word in key_words:
                    if re.search(word,final_text,re.IGNORECASE) :
                        print("matched_word: ",word)
                        # get_email(variables,garbage_emails,name,link,email,response)
