...
def chongqing():
    url = "http://www.cs.cqu.edu.cn/ENGLISH/Faculty1/Big_Data_Intelligence_and_Service_Computing_Team.htm"
    ...
    if response.status_code == 200:
        ...
        csv_filename = "Chongqing University.csv"
        ...
        univ_name = "Chongqing University"
        country = "China (Mainland)"

        garbage_emails = ['rencai@cqu.edu.cn'] # add garbage emails to this list

        variables = [csvwriter1, csvwriter2, univ_name, country, garbage_emails]
        d = soup.find('div', {'class': 'con'})
        dd = d.find_all('div',{'class':'xi'})
        print(len(dd))
        for i in dd:
            prof_type = i.find_all('ul')
            for j in prof_type:
                profs = j.find_all('li')
                for prof in profs:
                    name_div = prof.find('a')
                    if name_div == None:
                        continue
                    name = name_div.get_text().strip()
                    link = "http://www.cs.cqu.edu.cn/"+name_div.get('href').strip("../..")
                    email = "Not Found"
                    print(name, link)
                    # return
                    try:
