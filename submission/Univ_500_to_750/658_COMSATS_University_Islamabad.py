...
def comsats():
    url = "https://ww2.comsats.edu.pk/cs/faculty.aspx"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "COMSATS University Islamabad.csv"
        ...
        univ_name = "COMSATS University Islamabad"
        country = "Pakistan"
        ...
        profs1 = soup.find_all('tr',{'class':'rgRow'})
        profs2 = soup.find_all('tr',{'class':'rgAltRow'})

        print(len(profs1))

        profs1.extend(profs2)
        print
        for prof in profs1:
            # font-size: 15pt
            span =prof.find('span',{'style':'font-size: 15pt'})
            # print('i1')
            if span == None:
                continue
            # print(span)
