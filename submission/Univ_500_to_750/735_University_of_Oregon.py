...
def oregon():
    url = "https://cas.uoregon.edu/directory/computer-science"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "University of Oregon.csv"
        ...
        univ_name = "University of Oregon"
        ...
        d = soup.find('div', {'class': 'view-content'})
        dd = d.find_all('div', {'class': 'listing__row'})

        for i in dd:
            #<h2 class="views-field views-field-lname my0 uo-text-header-2"><span class="field-content"><a href="/directory/computer-science/all/arnita">Arnita Albertson</a></span></h2>
            h2 = i.find('h2', {'class': 'views-field views-field-lname my0 uo-text-header-2'})
            if h2 == None:
                continue
            a = h2.find('a')
            if a == None:
            ...
            link = "https://cas.uoregon.edu" + a.get('href')
            email = "Not Found"
            print(name, link)
            try:
