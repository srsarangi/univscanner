...
def abai():
    url = "https://www.kaznpu.kz/en/129/page/"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Abai Kazakh National Pedagogical University.csv"
        ...
        univ_name = "Abai Kazakh National Pedagogical University"
        country = "Kazakhstan"
        ...
        d = soup.find_all('tr',{'valign':'top'})
        # print(len(d))
        for i in d:
            a = i.find('a')
            if a== None:
                continue
            link = "https://www.kaznpu.kz"+ a.get('href')
