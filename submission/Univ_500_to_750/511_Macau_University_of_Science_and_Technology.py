...
def keyword_in_pdf(url, keyword):
    # Step 1: Stream the PDF directly from the URL
    response = requests.get(url, stream=True)
    if response.status_code == 200:
        # Step 2: Open the PDF from the streamed content
...
# JUST TEMPLATE 
# TAKING MORE TIME TO USe
def macau():
    url = "https://www.must.edu.mo/en/fhtm/staff"
    return
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Macau University of Science and Technology.csv"
        ...
        univ_name = "Macau University of Science and Technology"
        country = "Macau SAR"
        ...
        #<tr>
        # <td><a href="/en/fhtm/staff/ben-k-goh">Ben K. Goh</a></td>
        # <td>Dean</td>
        # <td><a href="mailto:kbgoh@must.edu.mo">kbgoh@must.edu.mo</a></td>
        # </tr>
        ...
        for i in d:
            ...
            if len(dd) < 2:
                continue
            name = dd[0].get_text()
            link = "https://www.must.edu.mo"+dd[0].get('href')
            email = dd[1].get_text()
            try:
...
def get_email(variables,garbage_emails,name,link,email,page_resp):
    ...
    page_resp = BeautifulSoup(page_resp.text, "html.parser")
    d = page_resp.find_all('a')
    ...
    for i in d:
        # get the name of the anchor tag
        if i.get_text() == "Download CV":
            pdf_url = "https://www.must.edu.mo"+i.get('href')
            for word in key_words:
                if keyword_in_pdf(pdf_url, word):
