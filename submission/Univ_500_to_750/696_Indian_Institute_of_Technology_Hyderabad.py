...
def iith():
    url = "https://cse.iith.ac.in/people/faculty.html"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Indian Institute of Technology Hyderabad.csv"
        ...
        univ_name = "Indian Institute of Technology Hyderabad"
        country = "India"

        garbage_emails = [] # add garbage emails to this list

        variables = [csvwriter1, csvwriter2, univ_name, country, garbage_emails]
        profs = soup.find_all('div',{'class':'col-sm-12 col-md-6 faculty-col'})
