...
def brno():
    url = "https://www.fit.vut.cz/units/upsy/staff/.en"
    response = requests.get(url)
    if response.status_code == 200:

        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Brno University of Technology.csv"
        ...
        univ_name = "Brno University of Technology"
        country = "Czech Republic"

        garbage_emails = ['info@fit.vut.cz'] # add garbage emails to this list

        variables = [csvwriter1, csvwriter2, univ_name, country, garbage_emails]
        d = soup.find_all('div', {'class': 'grid__cell size--12-12'})
        print(len(d))
        d1 = soup.find_all('div', {'class': 'grid__cell size--t-6-12'})
        d.extend(d1)
        for i in d:
            a = i.find_all('a')
            for prof in a:
