...
def ntu(url):
    # url = "https://www.ntu.ac.uk/search?category=Staff&meta_relatedBucketPage_sand=817042&start_rank=1"
    ...
    if response.status_code == 200:
        ...
        csv_filename = "Nottingham Trent University.csv"
        ...
        univ_name = "Nottingham Trent University"
        country = "United Kingdom"

        garbage_emails = ['enquiries@ntu.ac.uk'] # add garbage emails to this list

        
        variables = [csvwriter1, csvwriter2, univ_name, country, garbage_emails]
        #ul, class=listing no-spacing flex-row
        d = soup.find('ul',{'class':'listing no-spacing flex-row'})
        if d == None:
            return False
        # li class="result clearfix
        dd = d.find_all('li',{'class':'result clearfix'})
        if len(dd) == 0:
            return False
        for prof in dd:
            h2 = prof.find('h2',{'class':'result-title skim-bottom'})
            if h2 == None:
                continue
            a = h2.find('a')
            if a == None:
...
if __name__ == '__main__':
    main_url = "https://www.ntu.ac.uk/search?category=Staff&meta_relatedBucketPage_sand=817042&start_rank="
    i = 1
    while True:
        print("page",i)
        if not ntu(main_url+str(i)):
            break
        else:
            i += 10
