...
def carolina():
    url = "https://sc.edu/study/colleges_schools/engineering_and_computing/departments/computer_science_and_engineering/our_people/index.php"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable

        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "University of South Carolina.csv"
        ...
        univ_name = "University of South Carolina"
        country = "United States"

        garbage_emails = [] # add garbage emails to this list

        variables = [csvwriter1, csvwriter2, univ_name, country, garbage_emails]
        d = soup.find('div', {'class': 'simple-list mod-directory'})
        dd = d.find_all('a')
        for i in dd:
            link ="https://sc.edu"+ i.get('href')
            div = i.find('div',{'class':'blurb'})
            if div == None:
                continue
