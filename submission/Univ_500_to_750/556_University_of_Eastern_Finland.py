...
def uaef():
    url = "https://www.uef.fi/en/unit/school-of-computing"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "University of Eastern Finland.csv"
        ...
        univ_name = "University of Eastern Finland"
        country = "Finland"
        ...
        d = soup.find_all('div', {'class': 'accordion-def'})
        print(len(d))
        # return 
        for i in d:
            dd = i.find('div', {'class': 'contact-card__grid'})
            if dd == None:
                continue
            profs = dd.find_all('a', {'class': 'contact-card__link'})
            for prof in profs:
                link = ""+ prof.get('href')
                name_div = prof.find('div', {'class': 'contact-card__content'})
                if name_div == None:
                    continue
                name = name_div.find('h3').get_text().strip()
                email = "Not Found"
                email_div = name_div.find('p', {'class': 'contact-card__email'})
                if email_div != None:
