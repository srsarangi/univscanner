...
def trieste():
    url = "https://dssc.units.it/lecturers.html"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "University of Trieste.csv"
        ...
        univ_name = "University of Trieste"
        country = "Italy"
        ...
        profs = soup.find_all('div',{'class':'col-md-9 views-fieldset'})
        for prof in profs:
            h2 = prof.find('h2')
            if h2 == None:
                continue
            name = h2.get_text().strip()
            div = prof.find('div',{'class':'views-field views-field-field-web-page'})
            if div == None:
                continue
            a = div.find('a')
            if a == None:
