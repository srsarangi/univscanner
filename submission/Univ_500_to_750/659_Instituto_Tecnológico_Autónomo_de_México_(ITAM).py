...
def itam(url):
    ...
    if response.status_code == 200:
        ...
        csv_filename = "Instituto Tecnológico Autónomo de México (ITAM).csv"
        ...
        for prof in profs:
            ...
            link = "https://politica.itam.mx" +a.get('href')
            email = "Not Found"
            print(name,link)
            try:
...
def get_email(variables,garbage_emails,name,link,email,page_resp):
    ...
    key_words.extend(key_words_spanish)
...
if __name__ == '__main__':
    main_url = "https://politica.itam.mx/en/taxonomy/term/4082?page="
    i = 0
    while True:
        print('page',i)
        if not itam(main_url+str(i)):
            break
        else:
            i += 1
