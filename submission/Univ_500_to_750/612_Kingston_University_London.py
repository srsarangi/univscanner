...
def kingston():
    url = "https://www.kingston.ac.uk/staff/search-results/faculty/faculty-of-engineering-computing-and-the-environment-5/school-of-computer-science-and-mathematics-15/department-of-computer-science-42/"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable

        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Kingston University, London.csv"
        ...
        univ_name = "Kingston University, London"
        country = "United Kingdom"

        garbage_emails = [] # add garbage emails to this list

        variables = [csvwriter1, csvwriter2, univ_name, country, garbage_emails]
        #single-col-row-heading staff-listing-name
        d = soup.find_all('h3',{'class':'single-col-row-heading staff-listing-name'})
        for i in d:
            a = i.find('a')
            if a == None:
                continue
            link = "https://www.kingston.ac.uk"+ a.get('href')
