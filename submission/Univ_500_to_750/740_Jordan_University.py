...
def jordan():
    url = "https://www.just.edu.jo/FacultiesandDepartments/it/Departments/cpe/Pages/DepartmentStaff.aspx"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Jordan University of Science & Technology.csv"
        ...
        univ_name = "Jordan University of Science & Technology"
        country = "Jordan"

        garbage_emails = [] # add garbage emails to this list

        variables = [csvwriter1, csvwriter2, univ_name, country, garbage_emails]
        d = soup.find_all('div', {'class': 'col-sm-6 col-md-3 member mb-3'})
        # for i in d:
        # print(len(d))
        for i in d:

            div = i.find('div', {'class': 'member-name'})
            if div == None:
                continue
            name = div.get_text().strip()
            e = i.find('div', {'class': 'member-email'})
            a = e.find('a')
            email = "Not Found"
            if a != None:
                email = a.get('href')[7:]
            l = i.find('div', {'class': 'card-footer text-center'})
            a1 = l.find('a')
            if a1 == None:
                continue
            link = "https://www.just.edu.jo" + a1.get('href')
