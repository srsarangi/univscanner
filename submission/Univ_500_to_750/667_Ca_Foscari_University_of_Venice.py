...
def foscari():
    url = "https://www.unive.it/data/en/7051/list-of-professors"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Ca' Foscari University of Venice .csv"
        ...
        univ_name = "Ca' Foscari University of Venice "
        country = "Italy"

        garbage_emails = [] # add garbage emails to this list

        variables = [csvwriter1, csvwriter2, univ_name, country, garbage_emails]
        profs = soup.find_all('div',{'class':'card-body'})
        for prof in profs:
            a = prof.find('a')
            if a == None:
                continue
            name = a.get_text().strip()
            link = "https://www.unive.it"+a.get('href')
            link = link.strip()
            print(name, link)
            try:
...
def get_email(variables,garbage_emails,name,link,email,page_resp):
    ...
    key_words = ['operating systems','Embedded System','embedded system', 'Operating Systems','embedded systems',"Embedded Systems",'kernel','Microcontroller Systems','Integrated Systems','Embedded Computing','Embedded Software','unix','linux']
