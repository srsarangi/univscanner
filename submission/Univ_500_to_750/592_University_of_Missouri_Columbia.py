...
def missouri():
    url = "https://engineering.missouri.edu/departments/eecs/eecs-faculty/"
    response = requests.get(url)
    if response.status_code == 200:

        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "University of Missouri, Columbia.csv"
        ...
        univ_name = "University of Missouri, Columbia"
        country = "United States"

        garbage_emails = ['engineering@missouri.edu'] # add garbage emails to this list

        variables = [csvwriter1, csvwriter2, univ_name, country, garbage_emails]
        # miz-wrapper miz-fill--white
        d = soup.find('div', {'class': 'miz-wrapper miz-fill--white'})
        dd = d.find('article', {'class': 'miz-main-grid__article eng-main-grid__article'})
        profs = dd.find_all('div',{'class':'miz-card__body'})
        for prof in profs:
            h4 = prof.find('h4')
            if h4 == None:
                continue
