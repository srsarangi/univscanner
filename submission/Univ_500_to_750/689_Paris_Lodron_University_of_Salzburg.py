...
def salzburg():
    url = "https://www.plus.ac.at/computer-science/staff/?lang=en"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Paris Lodron University of Salzburg.csv"
        ...
        univ_name = "Paris Lodron University of Salzburg"
        ...
        d = soup.find('div',{'class':'columns post_content'})
        print(len(d))
        dd = d.find_all('a')
        for i in dd:
            # a = i.find('a')
