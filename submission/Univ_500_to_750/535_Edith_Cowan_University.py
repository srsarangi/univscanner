...
def cowan():
    url = "https://cet.ecu.edu/csci/about-us/faculty-staff/"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Edith Cowan University .csv"
        ...
        univ_name = "Edith Cowan University "
        country = "Australia"
        ...
        # print(len(dd))
        for i in dd:
            each_info = i.find_all('div', {'class': 'mb-2 col-md-4'})
            # print(len(each_info))
            for info in each_info:

                name = info.find_all('strong')
                if len(name) == 0:
                    continue
