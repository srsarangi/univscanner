...
def adolfo(url):
    # url = "https://pure.uai.cl/en/persons/?organisationIds=b8ea1a77-b7a7-4ff3-83a3-499b71818547&nofollow=true"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        print("entered")
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Universidad Adolfo Ibáñez.csv"
        ...
        univ_name = "Universidad Adolfo Ibáñez"
        country = "Chile"
        ...
        d = soup.find('div', {'class': 'portal-search-results'})
        if d == None:
            print("No data found")
            return False
        dd = d.find('ul',{'class':'grid-results'})
        if dd == None:
            return False
        ...
        # return 
        for prof in profs:
            name_div = prof.find('div',{'class':'result-container'})
            if name_div == None:
                continue
            h3 = name_div.find('h3',{'class':'title'})
            if h3 == None:
                continue
            a = h3.find('a')
            if a == None:
...
if __name__ == '__main__':
    # adolfo()
    main_url = "https://pure.uai.cl/en/persons/?nofollow=true&page="
    i = 0
    while True:
        print("page: ", i)
        if not adolfo(main_url + str(i)):
            break
        else:
            i += 1
