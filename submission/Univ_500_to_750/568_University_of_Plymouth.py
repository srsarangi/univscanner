...
def plymouth():
    url = "https://www.plymouth.ac.uk/schools/school-of-engineering-computing-and-mathematics/computer-science"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        # print("entered")
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "University of Plymouth.csv"
        ...
        univ_name = "University of Plymouth"
        ...
        d = soup.find('div', {'class': 'staff photo-strip'})
        if d == None:
            print("No data found")
            return 
        dd = d.find_all('li',{'class':'thumbnail'})
        if len(dd) == 0:
            print("No data found")
            return
        for i in dd:
            ...
            if prof == None:
                continue
            link = "https://www.plymouth.ac.uk"+prof.get('href')
