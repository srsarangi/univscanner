...
def compostela():
    url = "https://www.usc.gal/en/department/electronics-and-computing/directory"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Universidade de Santiago de Compostela.csv"
        ...
        univ_name = "Universidade de Santiago de Compostela"
        country = "Spain"
        ...
        profs = soup.find_all('div',{'class':'generic-summary-content-wrapper'})
        for prof in profs:
            a = prof.find('a')
            if a == None:
                continue
            link = "https://www.usc.gal"+ a.get('href')
