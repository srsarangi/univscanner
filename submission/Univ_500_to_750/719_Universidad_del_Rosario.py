...
def rosario():
    url = "https://pure.urosario.edu.co/en/persons/?organisationIds=4888a292-0dff-4636-9611-ac98bbbddbb2&nofollow=true"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Universidad del Rosario.csv"
        ...
        univ_name = "Universidad del Rosario"
        ...
        profs = soup.find_all('div',{'class':'rendering rendering_person rendering_short rendering_person_short'})
        for prof in profs:
            a = prof.find('a',{'rel':'Person'})
            if a == None:
                continue
            link = a.get('href')
