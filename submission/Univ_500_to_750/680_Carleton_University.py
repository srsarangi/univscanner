...
def carleton():
    url = "https://carleton.ca/scs/our-people/school-of-computer-science-faculty/"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Carleton University.csv"
        ...
        univ_name = "Carleton University"
        country = "Canada"

        garbage_emails = [] # add garbage emails to this list

        variables = [csvwriter1, csvwriter2, univ_name, country, garbage_emails]
        profs = soup.find_all('div',{'class':'card card--no-underline card--people-list'})
        for prof in profs:
            a = prof.find('a',{'class':'card__link equalheight'})
            if a == None:
                continue
            link = a.get('href')
            h3 = prof.find('h3')
