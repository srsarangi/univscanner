...
def haifa():
    url = "https://cs.hevra.haifa.ac.il/index.php/en/people-en/faculty-en"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "University of Haifa.csv"
        ...
        profs = soup.find_all('div',{'class':'sppb-column'})
        print(len(profs))
        for prof in profs:
            div = prof.find('div',{'class':'sppb-person-information'})
            if div == None:
                continue
            # print('1')
            span = div.find('span',{'class':'sppb-person-name'})
            if span == None:
                continue
            # print('2')
            name = span.get_text().strip()
            email_div = div.find('a')
            email = "Not Found"
            if email_div != None:
                email = email_div.get('href')[7:]

            p = prof.find('p',{'style':'text-align: center;'})
            if p == None:
                continue
            a = p.find('a')
            if a == None:
