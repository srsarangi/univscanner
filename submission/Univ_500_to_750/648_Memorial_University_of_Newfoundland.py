...
def newfoundland():
    url = "https://www.mun.ca/computerscience/our-people/"
    response = requests.get(url)
    if response.status_code == 200:

        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Memorial University of Newfoundland.csv"
        ...
        for prof in profs:
            ...
            if "http" not in a.get('href') and "www" not in a.get('href'):
                link = "https://www.mun.ca"+a.get('href')
            else:
                link = a.get('href')
