...
def cgu():
    url = "https://csie.cgu.edu.tw/p/412-1086-11859.php?Lang=en"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Chang Gung University.csv"
        ...
        univ_name = "Chang Gung University"
        country = "Taiwan"
        ...
        # mrow container
        d = soup.find('div', {'class': 'mrow container'})
        if d == None:
            print("data not found")
            return False
        dd = d.find_all('div', {'class': 'department'})
        for i in dd:
            profs = i.find_all('div', {'class': 'teacher'})
            for prof in profs:
                name_div = prof.find('div', {'class': 'intro'})
                if name_div == None:
                    continue
