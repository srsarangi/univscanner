...
def alfaisal():
    url = "https://cos.alfaisal.edu/en/maths-cs-faculty-staff"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Alfaisal University.csv"
        ...
        dd = soup.find_all('div', {'class': 'faculty col-md-4 col-sm-6 col-xs-12'})

        for i in dd:
            n = i.find('div', {'class': 'facultyName'})
            if n == None:
                continue
            name = n.get_text().strip()
            desd = i.find('div', {'class': 'facultyDesignation'})
            if desd == None:
                continue
            a = desd.find('a')
            if a == None:
