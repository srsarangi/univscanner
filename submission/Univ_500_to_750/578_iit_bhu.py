...
def iit_bhu():
    url = "https://iitbhu.ac.in/dept/cse/faculty"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Indian Institute of Technology (BHU) Varanasi.csv"
        ...
        univ_name = "Indian Institute of Technology (BHU) Varanasi"
        country = "India"
        ...
        d1 = d.find('div', {'class': 'row'})
        d2 = d.find_all('div', {'class': 'row py-5'})
        # print(len(d2))
        d2.append(d1)
        # print(len(d2))
        for i in d2:
            div = i.find_all('div', {'class': 'col-md-4'})
            if len(div) == 0:
                continue
            for prof in div:
                a = prof.find('a')
                if a == None:
                    continue
                # print(a)
                link = "https://iitbhu.ac.in"+a.get('href')
