...
# JUST TEMPLATE
def asub():
    url = "https://www.asu.edu.jo/en/it/cs/Pages/Department-Members.aspx"
    return
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Applied Science University - Bahrain.csv"
        ...
        univ_name = "Applied Science University - Bahrain"
        country = "Bahrain"
        ...
        d = soup.find('div', {'class': 'content-wrapper member'})
        dd = d.find('ul')
        profs = dd.find_all('li')
        for prof in profs:
            name_div = prof.find('div', {'class': 'staff-info-text'})
            if name_div == None:
                continue
            h3 = name_div.find('h3')
            if h3 == None:
