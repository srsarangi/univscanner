...
def saarlandes():
    url = "https://saarland-informatics-campus.de/en/forschung-research/advisors/"
    response = requests.get(url)
    if response.status_code == 200:

        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Universität des Saarlandes.csv"
        ...
        univ_name = "Universität des Saarlandes"
        country = "Germany"
        ...
        if d == None:
            ...
            return False
        dd = d.find_all('div', {'class': 'row CS_person'})
        print(len(dd))
        for i in dd:
            profs = i.find_all('div', {'class': 'col-md-4'})
            for prof in profs:
                name_div = prof.find('div', {'class': 'CS_person_name'})
                if name_div == None:
                    continue
                ...
                if len(a) == 2:
                    email = a[0].get('href')
                    #javascript:unscrambleEmailAddr("meeapel@cs.uni-saarland.d")
                    # get the email from above
                    email = email.split('"')[1][3:]
                    # print(email)
                    link = a[1].get('href')
