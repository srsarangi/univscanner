...
def njit():
    url = "https://cs.njit.edu/people"
    response = requests.get(url)
    if response.status_code == 200:

        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "New Jersey Institute of Technology (NJIT).csv"
        ...
        univ_name = "New Jersey Institute of Technology (NJIT)"
        country = "United States"

        garbage_emails = [] # add garbage emails to this list

        variables = [csvwriter1, csvwriter2, univ_name, country, garbage_emails]
        d = soup.find_all('div',{'class':'faculty-cards'})
        for i in d:
            a = i.find_all('a',{'class':'column'})
            for prof in a:
                link ="https:" +prof.get('href')
                h1 = prof.find('h1',{'class':'name'})
                if h1 == None:
                    continue
