...
def palacky():
    url = "https://www.inf.upol.cz/lide"
    response = requests.get(url)
    if response.status_code == 200:

        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Palacký University Olomouc.csv"
        ...
        univ_name = "Palacký University Olomouc"
        country = "Czech Republic"

        garbage_emails = [] # add garbage emails to this list

        variables = [csvwriter1, csvwriter2, univ_name, country, garbage_emails]
        d = soup.find_all('div', {'class': 'box box-2 box--no-padding people'})
        # print(len(d))
        for prof in d:
            link_div = prof.find('a')
            if link_div == None:
                continue
            link = link_div.get('href')
            name_div =prof.find('h3')
            if name_div == None:
                continue
            name = name_div.get_text().strip()
            email = "Not Found"
            email_div= prof.find('td', {'class': 'people__card--email'})
            if email_div != None:
