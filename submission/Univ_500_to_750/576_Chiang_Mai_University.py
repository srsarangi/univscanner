...
def chiang():
    url = "https://www.cs.science.cmu.ac.th/academicstaff/"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Chiang Mai University.csv"
        ...
        univ_name = "Chiang Mai University"
        country = "Thailand"
        ...
        d = soup.find('div', {'class': 'elementor-widget-wrap elementor-element-populated'})
       
        dd = d.find_all('p', {'class': 'elementor-image-box-description'})
        print(len(dd))
        for i in dd:
            info = i.find_all('b')
            # print(len(info))
            if len(info) != 3:
                continue
            name = info[0].get_text().strip('Tel:').strip()
            email = "Not Found"
            # print(name)
            div = i.find('a')
            if div == None:
