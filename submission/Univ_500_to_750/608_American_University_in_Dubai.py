...
def auid():
    url = "https://www.aud.edu/university-overview/faculty/"
    response = requests.get(url)
    if response.status_code == 200:

        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "American University in Dubai.csv"
        ...
        univ_name = "American University in Dubai"
        ...
        # <div class="simple-content" id="tesd">
        d = soup.find('div', {'class': 'simple-content', 'id': 'tesd'})
        dd = d.find_all('div', {'class': 'row'})
        for i in dd:
            profs = i.find_all('div', {'class': 'col-sm-6'})
            for prof in profs:
                name_div = prof.find('div', {'class': 'person__name'})
                if name_div == None:
                    continue
                ...
                if a == None:
                    continue
                link = "https://www.aud.edu"+ a.get('href')
                print(name, link)
                email = "Not Found"
                try:
