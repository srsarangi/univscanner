...
def oklahoma():
    ...
    if response.status_code == 200:
        ...
        csv_filename = "University of Oklahoma.csv"
        ...
        univ_name = "University of Oklahoma"
        country = "United States"

        garbage_emails = ['cs@ou.edu'] # add garbage emails to this list

        variables = [csvwriter1, csvwriter2, univ_name, country, garbage_emails]

        d = soup.find_all('div',{'class':'card-component section'})
        # print(len(d))
        for prof in d:
            info = prof.find('div',{'class':'card-body'})
            if info == None:
                continue
            name_div = info.find('div',{'class':'card-title h5'})
            if name_div == None:
                continue
            name = name_div.get_text().strip()
            link_div = prof.find('div',{'class':'card-footer'})
            if link_div == None:
                continue
            link_a = link_div.find('a')
            if link_a == None:
                continue
            if "http" not in link_a.get('href') and 'www' not in link_a.get('href'):
                link = "https://www.ou.edu"+ link_a.get('href')
            else:
...
def get_email(variables,garbage_emails,name,link,email,page_resp):
    ...
    key_words = ['embedded','operating systems','Embedded System','embedded system', 'Operating Systems','operating system','Operating System','embedded systems',"Embedded Systems",'kernel','Microcontroller Systems','Integrated Systems','Embedded Computing','Embedded Software','unix','linux']
