...
def fribourg():
    url = "https://www.unifr.ch/inf/en/all.html"
    response = requests.get(url)
    if response.status_code == 200:
        
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Université de Fribourg.csv"
        ...
        univ_name = "Université de Fribourg"
        country = "Switzerland"

        garbage_emails = ['diuf-secr@unifr.ch'] # add garbage emails to this list

        variables = [csvwriter1, csvwriter2, univ_name, country, garbage_emails]
        #div class="campusviews--list peoplelist
        d = soup.find('div', {'class': 'campusviews--list peoplelist'})
        if d == None:
            print("No data found")
            return 
        #article class="box bg-white shadow
        dd = d.find_all('article',{'class':'box bg-white shadow'})
        if len(dd) == 0:
            print("No data found")
            return
        for i in dd:
