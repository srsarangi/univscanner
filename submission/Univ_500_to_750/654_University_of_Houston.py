...
def houston(url):
    ...
    if response.status_code == 200:
        ...
        csv_filename = "University of Houston.csv"
        ...
        d = soup.find('section',{'id':'content-well'})
        profs = d.find_all('div',{'class':'row'})
        print(len(profs))
        for prof in profs:
            name_div = prof.find('div',{'class':'col-sm-7'})
            if name_div == None:
                continue
            a = name_div.find('a',{'rel':'noopener'})
            if a == None:
                continue
            name = a.get_text().strip()
            link = a.get('href')
            email = "Not Found"
            email_div = prof.find('div',{'class':'col-sm-5'})
...
if __name__ == '__main__':
    url1 = "https://uh.edu/nsm/computer-science/people/faculty/"
    url2 = "https://uh.edu/nsm/computer-science/people/faculty-emeriti/"
    houston(url1)
    houston(url2)
