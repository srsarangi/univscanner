...
def pieas():
    url = "http://old.pieas.edu.pk/dcis/faculty.cshtml"
    response = requests.get(url)
    if response.status_code == 200:

        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Pakistan Institute of Engineering and Applied Sciences (PIEAS).csv"
        ...
        univ_name = "Pakistan Institute of Engineering and Applied Sciences (PIEAS)"
        country = "Pakistan"
        ...
        d = soup.find('div',{'class':'static_content'})
        profs = d.find_all('div',{'class':'note'})
        for prof in profs:
            name_div = prof.find('b')
            if name_div == None:
                continue
            name = name_div.get_text().split(" (")[0].strip()
            link = url
            email = "Email Not Found"
            print(name,link)
            prof_text = prof.get_text()
