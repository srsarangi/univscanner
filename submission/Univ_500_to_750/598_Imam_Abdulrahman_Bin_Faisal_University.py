...
def iau(url):
    # url = "https://www.iau.edu.sa/en/colleges/college-of-computer-science-and-information-technology/faculty?search_api_views_fulltext=&page=1"
    response = requests.get(url)
    if response.status_code == 200:

        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Imam Abdulrahman Bin Faisal University (IAU) (formerly UNIVERSITY OF DAMMAM).csv"
        ...
        univ_name = "Imam Abdulrahman Bin Faisal University (IAU) (formerly UNIVERSITY OF DAMMAM)"
        ...
        d = soup.find_all('div', {'class': 'block-list-row'})
        if len(d) ==0:
            return False
        for i in d:
            h3 = i.find('h3', {'class': 'teaser-title section-title'})
            if h3 == None:
                continue
            a = h3.find('a')
            if a == None:
            ...
            link = "https://www.iau.edu.sa"+a.get('href')
            email = "Not Found"
            print(name,link)
            try:
        ...
        if re.search(word,prof_text,re.IGNORECASE) or word == 'BASE':
            ...
            if email != "Not Found": # if email is already found, no need to find it again (at line 50)
                ...
                if len(new_emails) == 0:
                    ...
                    prof_email = new_emails[0].split("Email")
                    if len(prof_email) == 2:
...
if __name__ == '__main__':
    main_url = "https://www.iau.edu.sa/en/colleges/college-of-computer-science-and-information-technology/faculty?search_api_views_fulltext=&page="
    i = 0
    while True:
        print('page',i)
        if not iau(main_url+str(i)):
            break
        else:
            i += 1
