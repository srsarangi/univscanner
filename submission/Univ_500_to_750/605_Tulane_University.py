...
def tulane(url):
    # url = "https://sse.tulane.edu/cs/faculty-0"
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable

        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Tulane University.csv"
        ...
        for prof in dd:
            ...
            if a == None:
                continue
            link ="https://sse.tulane.edu"+ a.get('href')
            # name = a.get_text().strip()
            name = prof.get_text().split(" | ")[0].strip()
            print(name, link)
            email = "Not Found"
            try:
...
if __name__ == '__main__':
    main_url = "https://sse.tulane.edu/cs/faculty-"
    i = 0
    while True:
        if not tulane(main_url + str(i)):
            break
        i += 1
