...
def phule():
    url = "http://www.unipune.ac.in/dept/science/computer_science/cs_webfiles/faculty.htm"
    return
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Savitribai Phule Pune University.csv"
        ...
        univ_name = "Savitribai Phule Pune University"
        country = "India"
        ...
        profs = soup.find_all('tr',{'style':'color:#333333;background-color:#F7F6F3;'})
