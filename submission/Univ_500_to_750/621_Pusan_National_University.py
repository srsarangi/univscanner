...
def pusan():
    url = "https://faculty.pusan.ac.kr/cseEng/46554/subview.do"
    response = requests.get(url)
    if response.status_code == 200:

        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "Pusan National University.csv"
        ...
        univ_name = "Pusan National University"
        country = "South Korea"
        ...
        d = soup.find('ul', {'class': '_wizOdr _prFlList'})
        if d == None:
            print("None")
            return
        profs = d.find_all('li')
        for prof in profs:
            div = prof.find('div', {'class': 'artclTitle'})
            if div == None:
                continue
            a = div.find('a')
            if a == None:
                continue
            link = "https://faculty.pusan.ac.kr" + a.get('href')
