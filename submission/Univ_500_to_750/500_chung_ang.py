# import requests
# import urllib.request
# import time
# import urllib
# import re
# import csv
# from bs4 import BeautifulSoup


# def chung_ang():
#     url = "https://cse.cau.ac.kr/eng/sub02/sub0201.php"
#     response = requests.get(url)
#     if response.status_code == 200:
#         soup = BeautifulSoup(response.text, "html.parser")
#         txt_filename = "chung_ang.txt"
#         csv_filename = "chung_ang.csv"
#         #write overall file later

#         f1 = open(txt_filename, "w")
#         f2 = open(csv_filename, "w")
#         csvwriter = csv.writer(f2)

#         univ_name = "Chung Ang University"
#         country = "South Korea"

#         garbage_emails = []

#         var = [f1, csvwriter, univ_name, country]

#         d = soup.find_all('div', {'class': 'summary'})
#         keywords = ['OS','operating systems','Embedded System']


#         for i in d:
#             # a = i.find('a')
#             # find h4 tag
#             name = i.find('h4').get_text(strip=True)
#             a = i.find('a')
#             link = a.get('href')
#             # check if link is valid or not
#             # valid_link = False
#             try:
#                 prof_resp = requests.get(link)
#                 # valid_link = True
#             except:
#                 link = "None"
#             ul = i.find('ul')
#             # find all li tags in ul
#             lis = ul.find_all('li')
#             lecture = lis[2].find('span').get_text()
#             if any(x in lecture for x in keywords):
#                 # print(name, link, lecture)
#                 email = lis[3].find('a').get_text()
#                 # print(email)
#                 # write link
#                 f1.write(link + '\n' + '\t\t' + name + '\t' + email +'\n')
#                 csvwriter.writerow([univ_name, country, name, email, link])
#                 print(univ_name, country, name, email, link)
#                 # print("length of name ", len(name))
                
#             # get the span tag
#         f1.close()
#         f2.close()
            
#     else:
#         print("Failed to connect to chung_ang!!")
#         return
    
# chung_ang()


















import requests
import urllib.request
import time
import urllib
import re
import csv
from bs4 import BeautifulSoup

def chung_ang():
    url = "https://cse.cau.ac.kr/eng/sub02/sub0201.php"
    response = requests.get(url)
    keywords = ['OS','operating systems','Embedded System']
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "chung_ang.csv"
        final_csv = "combined.csv"
        
        f1 = open(csv_filename, "w") # write data to the csv
        csvwriter1 = csv.writer(f1)

        f2 = open(final_csv, "a") # append data to the combined csv
        csvwriter2 = csv.writer(f2)

        univ_name = "Chung Ang University"
        country = "South Korea"

        garbage_emails = [] # add garbage emails to this list

        variables = [csvwriter1, csvwriter2, univ_name, country, garbage_emails]

        # GET DATA
        d = soup.find_all('div', {'class': 'summary'})

        #interating 0ver data

        for i in d:

            # start email as not found, if found keep the email
            # a = i.find('a')
            name = i.find('h4').get_text(strip=True)
            a = i.find('a')
            link = a.get('href')
            email = "Not Found"
            # check if link is valid or not
            try:
                prof_resp = requests.get(link)
            except:
                prof_resp = "None"
            ul = i.find('ul')
            # find all li tags in ul
            lis = ul.find_all('li')
            lecture = lis[2].find('span').get_text()
            if any(x in lecture for x in keywords):
                # print(name, link, lecture)
                email = lis[3].find('a').get_text()
                print(email)
                print(name, link)
                get_email(variables,garbage_emails,name,link,email,prof_resp)
            
        f1.close()
        f2.close()
        print("Finished")



def get_email(variables,garbage_emails,name,link,email,page_resp):
    csvwriter1, csvwriter2, univ_name, country, garbage_emails = variables
    if email != "Not Found": # if email is already found, no need to find it again (at line 50)
        # just write
        csvwriter1.writerow([univ_name,country,name,email,link])
        csvwriter2.writerow([univ_name,country,name,email,link])
        return
    prof_soup = BeautifulSoup(page_resp.text, "html.parser")
    key_words = ['BASE'] # base is used when there is no use of keywords
    prof_text = prof_soup.text   # get the text from the page

    for word in key_words:
        if re.search(word,prof_text) or word == 'BASE':
            print("matched_word: ",word)
            if email != "Not Found": # if email is already found, no need to find it again (at line 50)
                # just write
                csvwriter1.writerow([univ_name,country,name,email,link])
                csvwriter2.writerow([univ_name,country,name,email,link])
            else:
                new_emails = list(re.findall(r"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,4}", prof_text))
                for em in garbage_emails:
                    if em in new_emails:
                        new_emails.remove(em)
                if len(new_emails) == 0:
                    email = "Email Not Found"
                    csvwriter1.writerow([univ_name,country,name,email,link])
                    csvwriter2.writerow([univ_name,country,name,email,link])
                else:
                    prof_email = new_emails[0]
                    csvwriter1.writerow([univ_name,country,name,prof_email,link])
                    csvwriter2.writerow([univ_name,country,name,prof_email,link])


                
            


if __name__ == '__main__':
    chung_ang()



            







        