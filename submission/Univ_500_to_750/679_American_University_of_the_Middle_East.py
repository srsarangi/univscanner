...
def aum():
    url = "https://www.aum.edu.kw/english/academics/deans-and-chairs"
    return
    response = requests.get(url)
    if response.status_code == 200:
        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "American University of the Middle East.csv"
        ...
        univ_name = "American University of the Middle East"
        country = "Kuwait"

        garbage_emails = [] # add garbage emails to this list

        variables = [csvwriter1, csvwriter2, univ_name, country, garbage_emails]
        # profs = soup.find_all('div',{'class':'infoHolder sameHeight'})
        # for prof in profs:
        #     div = prof.find('div',{'class':'mainInfo'})
        #     a = 
