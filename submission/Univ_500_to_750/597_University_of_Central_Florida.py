...
def ucf():
    url = "https://www.cs.ucf.edu/people/faculty/"
    response = requests.get(url)
    if response.status_code == 200:

        # scrappable
        soup = BeautifulSoup(response.text, "html.parser")
        csv_filename = "University of Central Florida.csv"
        ...
        univ_name = "University of Central Florida"
        ...
        d = soup.find('ul', {'class': 'list-unstyled row ucf-post-list-items'})
        if d == None:
        ...
        for i in dd:
            ...
            if a == None:
                continue
            link = a.get('href')
            strong = a.find('strong', {'class': 'mt-2 mb-1 d-block person-name'})
            if strong == None:
                continue
