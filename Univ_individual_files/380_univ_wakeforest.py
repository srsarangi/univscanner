...
def wakeforest():
    url = "https://cs.wfu.edu/about/faculty-staff/"   # homepage url
    ...
    filename = "wakeforest.txt"
    f = open(filename, "w")

    excel_filename = "wakeforest.csv"
    ...
    dd = soup.find_all('div',{'class': 'et_pb_team_member_description'})
    #d = d1.find_next('div')
    ...
    for i in dd:
        if i.find('h4') == None:
            continue        
        a = i.find('h4')                     # a contains the name and the homepage of prof
        #print(a)
        d = i.find('div')
        try:
            aa = d.find_all('a')
