...
def tsukuba():
    ...
    filename = "tsukuba.txt"
    f = open(filename, "w", encoding='utf-8')

    excel_filename = "tsukuba.csv"
    ...
    d = soup.find_all('tbody')
    k = soup.find_all('h3')
    index = None
    for i, heading in enumerate(k):
        if heading.text.strip().lower() == "computer architecture":
            index = i
            break
    #print(k)
    #print(len(k))
    ...
    for i in d:
        ...
        #print(z)
        #print(a,z)
        name = i.find('a')                                     # extracting prof name
        if(name==None):
            continue
        link = name.get('href')
        #print(name)
