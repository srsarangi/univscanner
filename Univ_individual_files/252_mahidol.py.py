...
def mahidol():
    url = "https://www.ict.mahidol.ac.th/people/staff-contact/computer-science-academic-group/"
    r = requests.get(url)
    ...
    filename = "mahidol.txt"
    f = open(filename, "w")

    excel_filename = "mahidol.csv"
    ...
    d = soup.find_all('div', {'class':"elementor-widget-wrap elementor-element-populated"})
    #print(d)
    b=0
    for i in d:
        a = i.find('h2', {'class':"elementor-heading-title elementor-size-default"})                     # a contains the name and the homepage of prof
        z = i.find('a', text = "Personal Website")
        #print(z)
        #print(a,z)
        if a == None:
            continue
        if( z == None):
            ...
            link = z.get('href')                # extracting prof page link
        #print(a.find('a'))
