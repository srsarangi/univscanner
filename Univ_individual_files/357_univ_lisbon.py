...
def lisbon():
    url = "https://ciencias.ulisboa.pt/pt/%C3%B3rg%C3%A3os-5#toc2"   # homepage url
    ...
    garbage_emails = ['info@ciencias.ulisboa.pt', 'mcalha@fc.ul.pt', 'anbessani@fc.ul.pt', 'pmveiga@fc.ul.pt', 'anbessani@fc.ul.pt']
    var = [f, csvwriter, csvwriter2, u_name, country]

    # d gives the array of all profs on the dept homepage
    # d = soup.find('a', {'class':"accordian-title"})
    dd = soup.find_all('a',{'target' : '_blank'})
    ...
    for i in dd:
        if i.get_text() == None:
            continue        
        name = i.get_text()                    # a contains the name and the homepage of prof
