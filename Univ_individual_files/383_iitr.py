...
def iitr():
    url = "https://www.iitr.ac.in/departments/CSE/pages/People+Faculty_List.html"   # homepage url
    ...
    dd = soup.find_all('div', {'class': 'list-wrapper'})
    ...
    for i in dd:
        if i.find_all('a') == None:
            continue        
        aa = i.find_all('a')                     # a contains the name and the homepage of prof
        name = aa[0].get_text()
        
        email = i.find('span', {'style':'float:left;text-align:left;'}).get_text()
       
        li = "https://www.iitr.ac.in"+ aa[0].get('href')                # extracting prof page link
        link = ""
        if len(aa) == 2:
            link = aa[1].get('href')
        else:
