...
def geoWashington():
    url = "https://www.cs.seas.gwu.edu/faculty"   # homepage url
    ...
    filename = "geoWashington.txt"
    f = open(filename, "w")

    excel_filename = "geoWashington.csv"
    ...
    d1 = soup.find('div', {"class" : 'field-item even', 'property': 'content:encoded'})
    ...
    dlink = []
    for i in dd:
        name = i.find('span' , {'style' : ['font-size: 16px;' , 'font-size:16px ;']})
        if name != None:
            dname.append(name.get_text())
        
        alinks = i.find_all('a')
        if alinks != None:    
            for j in alinks:
                if j.get_text() == 'View full profile':
                    dlink.append(j.get('href'))
    ...
    for i in range(len(dname)):
        t = dlink[i]
        if t[0:6]  == "/node/":
            dlink[i] = "https://www.cs.seas.gwu.edu" + dlink[i]
        print(dname[i], dlink[i])
        
	
        # check if link is valid on not
        try:    
            prof_resp = requests.get(dlink[i])        
        except:
            continue
        ...
        filterandgetEmail(var, garbage_emails, dname[i], dlink[i], email, prof_resp)
