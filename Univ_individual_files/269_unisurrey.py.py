...
def unisurrey():
    url = "https://www.surrey.ac.uk/department-computer-science/people/academic-staff"
    ...
    country = "England"
    garbage_emails = ['office@esa.tu-darmstadt.de']
    var = [f, csvwriter, csvwriter2, u_name, country]

    
    d = soup.find_all('a', {'class':'d-flex flex-lg-column flex-xl-row border border-top-0 border-right-0 border-left-0 border-mustard pb-3 mb-3'})
    ...
    for i in d:
        ...
        link = 'https://www.surrey.ac.uk'+i.get('href')
        
        #print(name)
        #name = name.get_text(" ")                                 # extracting prof page link
        name=name.strip()
