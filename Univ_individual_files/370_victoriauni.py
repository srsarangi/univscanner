...
def victoriauni():
    ...
    filename = "victoriauni.txt"
    f = open(filename, "w", encoding='utf-8')

    excel_filename = "victoriauni.csv"
    ...
    for i in d:
        try:
            link1 = i.find('a',{'class':'external'}).get('href')
        except:
            continue
        #print(link)
        name = i.find('div',{'class':'profile-name'}).get_text()
        email = i.find('div',{'class':'profile-email'}).get_text()
        #print(name)
        #print(link)
