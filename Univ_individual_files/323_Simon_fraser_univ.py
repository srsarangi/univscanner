...
def Simon_fraser_univ():
    url = "https://www.sfu.ca/computing/people/faculty.html"   # homepage url
    ...
    filename = "Simon_fraser_univ.txt"
    f = open(filename, "w")

    excel_filename = "Simon_fraser_univ.csv"
    ...
    d = soup.find('div',{"class" : "parsys_column cq-colctrl-lt0 people faculty-list"})
    dd = d.findAll('div',{"class":"text"})
    #print(dd)
    ...
    for i in dd:
      ...
      #  print(a)
       # td = i.find('td')
        ...
        #if a !=None:  
           # print(td)       # a contains the name and the homepage of prof
        link = "https://www.sfu.ca/"+ a.get('href')    
                  # extracting prof page link
        name = i.find('h4')                 # extracting prof name  
        naame=name.get_text();  
        name = naame.strip()
       # print(link+" "+name)
