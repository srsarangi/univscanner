...
def tuftus():
    url = "https://engineering.tufts.edu/cs/people/faculty"
    r = requests.get(url)
    ...
    d = soup.find_all('div',{'class':'entity entity-paragraphs-item paragraphs-item-faculty-by-status'})
    d = d[0].find_all('div', {'class':'profile section--gray-light margin-bottom match-height'})
    ...
    for i in d:
        ...
        #print(z)
        #print(a,z)
        name = i.find('div', {'class':'profile__title'})
        name = name.find('a')
        link1 = 'https://engineering.tufts.edu' + name.get('href')              # extracting prof page link
        #print(a.find('a'))
        
        name = name.get_text()                # extracting prof name
        #print(link)
        #print(name)
        email = i.find('span', {'class':'email-address'})
        email =email.find('a')
        email=email.get_text()
        b=b+1
        #print(b)
        prof_resp = requests.get(link1)
        soup1 = BeautifulSoup(prof_resp.text, "html.parser")
        link = soup1.find('a', text='Research website')
        try:
            link = link.get('href')
            #time.sleep(2)
        except:
