...
def isarael():
    ...
    url = "https://www.cs.technion.ac.il/people/faculty/"   # homepage url
    ...
    d = soup.find('div', {"class":"fm_wrapper1"})
    dd = d.find_all('div', {'class': 'fm_data'})
   # print(dd)

    #iterating for every prof
    for i in dd:
        name = i.find('div',{'class':'fm_name'}).get_text()
        print(name)
        a = i.find('a')  
        if a!=None:
            mail1 = a.find_next('a') 
        print(mail1)  
        if mail1!=None :                # a contains the name and the homepage of prof
            mail = mail1.get('href') 
            link =mail1.find_next('a').get('href')               # extracting prof page link
