...
def sduni():
    url = "https://www.sdu.dk/en/om_sdu/institutter_centre/imada_matematik_og_datalogi/medarbejdere"
    ...
    u_name = "Southern Denmark University"
    country = "Denmark"
    garbage_emails = ['contact@massey.ac.nz','imada@imada.sdu.dk']
    var = [f, csvwriter, csvwriter2, u_name, country]

    #print(soup)
    d = soup.find('tbody',{'class':'list'})
    
    e = d.find_all('a',{'class':'sort-surname'})
    d = d.find_all('a', {'class':'sort-name'})
    #print(len(d))
    #print(len(e))
    #d = d.find_all('a')
    #print(len(d))
    ...
    for i in d:
        ...
        name = i.get_text() + e[b].get_text()
        #print(name)
        link1 = 'https://www.sdu.dk' + i.get('href')
        #print(link)
