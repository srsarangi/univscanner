...
def univ_jyvaskyla():
    url = "https://www.jyu.fi/it/fi/tiedekunta/henkilosto/henkilosto#professorit-professors"   # homepage url
    ...
    filename = "Jyvaskyla.txt"
    f = open(filename, "w")

    excel_filename = "Jyvaskyla.csv"
    ...
    d = soup.find_all('div', {'class':'table-container'})
    x = d[5]
    dd = x.find('tbody')
    ddd = dd.find_all('tr')
...
def filterandgetEmail(var, garbage_emails, name, link, email, prof_resp):
   ...
   # text = TextBlob(prof_soup.text)
   # research_text=text.translate(to="en")
    print(research_text)
