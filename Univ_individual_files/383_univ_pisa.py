...
def pisa():
    url = "https://di.unipi.it/en/people/"   # homepage url
   ...
   # print(dd)
    ...
    for i in dd:
        ...
        n = i.find_all('td', {'style' : 'width: 20%'})
        if len(n) != 2:
            continue
        name = n[0].get_text() + " " + n[1].get_text() 
        email = aa[0].get('data-name') + "@" + aa[0].get('data-domain') +"." + aa[0].get('data-tld')
        link = aa[1].get('href')               # extracting prof page link
