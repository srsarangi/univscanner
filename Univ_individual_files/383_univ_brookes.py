...
def brookes():
    url = "https://www.brookes.ac.uk/ecm/about/staff/"   # homepage url
    ...
    filename = "brookes.txt"
    f = open(filename, "w")

    excel_filename = "brookes.csv"
    ...
    dd = soup.find_all('div', {'class': 'col-xs-12 col-sm-4 col-md-4 col-lg-3'})
    #print(dd)
	
    #iterating for every prof
    for i in dd:
        if i.find('h3', {'class':'sm-title hblack'}) == None:
            continue        
        n = i.find('h3', {'class':'sm-title hblack'})                     # a contains the name and the homepage of prof
        a = i.find('a')
        link = "https://www.brookes.ac.uk"+ a.get('href')                # extracting prof page link
