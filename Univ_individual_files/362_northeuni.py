...
def northeuni():
    url = "https://www.khoury.northeastern.edu/about/people/"
    ...
    filename = "northeuni.txt"
    f = open(filename, "w", encoding='utf-8')

    excel_filename = "northeuni.csv"
    ...
    #print(soup)
    d = soup.find('div',{'class':'ContentTypes grid-container grid-80 grid-parent grid-lines'})
    d = d.find_all('a')
    #print(d)
    #d = d.find_all('p')
    ...
    for i in d:
        link1 = i.get('href')
        name = i.find('div', {'class':'headline interactive'}).get_text()
        #print(name)
        #print(link1)
        #print(name.get_text())
        if i == None:
        ...
        try:
            ...
            continue
        prof_resp1 = BeautifulSoup(prof_resp.text, "html.parser") 
        name = prof_resp1.find('div', {'class': 'headline'}).get_text()
        try:
            email = prof_resp1.find('div',{'class':'extras'}).find('a').get('href')
        except:
            continue
        print(name,email)
        #link = link.get('href')
