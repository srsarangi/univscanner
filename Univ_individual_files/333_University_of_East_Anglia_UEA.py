...
def oxford():
    url = "https://www.uea.ac.uk/about/school-of-computing-sciences"   # homepage url
    ...
    d = soup.find('div', {'class':'slick-list draggable'})
    dd = soup.find_all('div',{'class':'person-slider__c-card'})
    # dd = d.find_all('div', {'class': 'views-field views-field-title'})

    #iterating for every prof
    for i in dd:
        a = i.find('a').find_next('a')                   # a contains the name and the homepage of prof
        link =  a.get('href')                # extracting prof page link
        name = i.find('h4').get_text()               # extracting prof name
        print(name, "\t", link)
        name = name.strip()
