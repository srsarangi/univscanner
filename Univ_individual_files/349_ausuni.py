...
def ausuni():
    ...
    #print(soup)
    d = soup.find('div',{'class':'panels-flexible-region panels-flexible-region-default_page-social panels-flexible-region-last right-sidebar-region'})
    link = d.find_all('a')
    #print(len(link))
    d = d.find_all('h4')
    #print(len(d))
    ...
    for i in d:
        ...
        name = i.get_text()
        link1 = 'https://www.aus.edu/'+link[b].get('href')
        #print(name,link1)
        #email = i.find_all('br')
        #.get_text()+'@um.edu.mo'
        #print(link1)
        ...
        #print(prof_resp)
        try:
            prof_resp = requests.get(link1,verify=False)      # requesting prof homepgae
        except:
            continue
