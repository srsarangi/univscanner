...
def norwegian():
    url = "https://www.ntnu.edu/idi/people/"   # homepage url
    ...
    dd = soup.find_all('a', {'class' : 'personList d-block'})
    
	
    #iterating for every prof
    for i in dd:
        if i.find('span', {'class': 'fullname'}) == None :
            continue        
        if i.get('href') == None:
            continue


        link = "https://www.ntnu.edu"+ i.get('href')                # extracting prof page link
        n = i.find('span', {'class': 'fullname'})                # extracting prof name
        name = n.get_text()
        name = name.strip()
        name = name.strip(",")
        email = ""
        em = i.find('span', {'class': 'email'})   
        if em == None:
            email =  "Not Found"
        else:
            email = em.get_text()
