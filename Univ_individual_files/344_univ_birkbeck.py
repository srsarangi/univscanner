...
def birkbeck():
    url = "https://search.bbk.ac.uk/search/people/?q=&departments=Department+of+Computer+Science+%26+Information+Systems"   # homepage url
    ...
    filename = "birkbeck.txt"
    f = open(filename, "w")

    excel_filename = "birkbeck.csv"
    ...
    while True:
        last = soup.find('li', {"class" : "next show-for-medium"})
        print(last)
        if last == None:
            break
        d.extend(soup.find_all('a', {'class' : 'row expanded'}))
        alast = last.find("a")
        url2 = alast.get('href')
        print(url2)
