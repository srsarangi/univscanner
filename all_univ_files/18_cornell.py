...
for i in d:
    ...
    timeout_links = ['http://www.cs.columbia.edu/~roxana', 'http://yulab.icmb.cornell.edu/', 'http://www.cs.cornell.edu/annual_report/00-01/bios.htm#demers']
    print(link)
    if link == None:
        continue
    ...
    for pattern in keyword_list:
        if re.search(pattern,research_text):
            flag = 0
            new_emails = set(re.findall(r"[A-Za-z._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,4}", prof_resp.text))
            if len(new_emails) == 0:
