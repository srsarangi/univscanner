import requests
import urllib.request
import time
import urllib
import re
import csv
from bs4 import BeautifulSoup

url = "https://www.cs.manchester.ac.uk/about/people/academic-and-research-staff/"   # homepage url
r = requests.get(url)                                        # request to url

# getting the soup by parsing the html parsel to text to request r
soup = BeautifulSoup(r.text, "html.parser")

# file initialization to write
filename = "uni_menchester.txt"
f = open(filename, "w")

excel_filename = "uni_menchester.csv"
f2 = open(excel_filename, "w")
csvwriter = csv.writer(f2)

overall_file = "all_emails.csv"
f3 = open(overall_file, "a")
csvwriter2 = csv.writer(f3)

u_name = "University of Menchester"
country = "England"

# using set() to store to remove duplicates emails
emails = set()

# d gives the array of all profs on the dept homepage
# d = soup.find('a', {'class':"accordian-title"})
d = soup.find_all('li', {'class':["tabrowwhite", "tabrowgrey"]})
# d2 = soup.find_all('li', {'class':"tabrowgrey"})

# print(d)  

#iterating for every prof
for i in d:
    a = i.find('a')                     # a contains the name and the homepage of prof
    if a == None:
        continue
    link = a.get('href')
    name = a.get_text()                 # extracting prof name
    name = name.replace("\t", "")
    name = name.replace("\n", "")
    try:
        prof_resp = requests.get(link)          # requesting prof homepgae
        prof_soup = BeautifulSoup(prof_resp.text, "html.parser") 
    except:
        continue

    print(name, link)
    pp = prof_soup.find('a', text='Personal Website')
    # print(pp)
    if pp == None:
        continue
    link2 = pp.get('href')
    print(name, link2)
    try:
        prof_resp2 = requests.get(link2)          # requesting prof homepgae
        prof_soup2 = BeautifulSoup(prof_resp2.text, "html.parser") 
    except:
        continue

    keyword_list = ['Computer architecture','computer architecture','Computer Architecture', 'Hardware And System Architecture', 'hardware and system architecture', 
                'Hardware and Architecture', 'hardware and architecture', 'embedded system', 'Embedded System','Computer Organization','VLSI', 'Computer and System']
    flag = 1
    research_text = prof_soup2.text
    for pattern in keyword_list:
        if re.search(pattern,research_text):
            flag = 0
            new_emails = set(re.findall(r"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,4}", prof_resp2.text))
            # new_emails2 = set(re.findall(r"[A-Za-z0-9._%+-]+&nbsp;&nbsp;(&commat;[A-Za-z0-9.-]+\.[A-Za-z]{2,4}", prof_resp.text))
            # new_emails = new_emails + new_emails2
            grabage_emails = ['mkearns@lehman.com', 'michael.kearns@bofasecurities.com', 'lhoot@seas.upenn.edu', 'kearmic@amazon.com']
            for eemail in grabage_emails:
                if eemail in new_emails:
                    new_emails.remove(eemail)

            # f.write(link + '\n' + name)
            if len(new_emails) == 0:
                email = "Email Not Found"
                f.write(link2 + '\n' + name + "\t"+ email + "\n")
                csvwriter.writerow([u_name, country, name, email, link2])
                csvwriter2.writerow([u_name, country, name, email, link])
            else:
                # f.write(link + '\n' + name)
                for email in new_emails:
                    f.write(link2 + '\n' + name + '\t\t' + email + '\n')
                    csvwriter.writerow([u_name, country, name, email, link2])
                    csvwriter2.writerow([u_name, country, name, email, link])
                # f.write("\n") 

            f.write(pattern)
            f.write('\n\n')
            break


f.close()
f2.close()
f3.close()
print("Finished")

